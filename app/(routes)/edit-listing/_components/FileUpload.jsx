        ...
        <div>
            <div className="flex items-center justify-center w-full">
                <label for="dropzone-file" className="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                    ...
                    <input id="dropzone-file" type="file" multiple onChange={handleFileUpload}
                    accept="image/png, image/gif, image/jpeg" 
                    className="hidden" />
                </label>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 
            lg:grid-cols-7 xl:grid-cols-10 gap-4 mt-4">
                {imagePreview.map((image,index)=>(
                    <div key={index}>
                        <Image src={image} width={120} height={120}
                        className="rounded-lg object-cover" alt="images"/>
                    </div>
                ))}
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 
            lg:grid-cols-7 xl:grid-cols-10 gap-4 mt-4">
                {imageList?.map((image,index)=>(
                    <div key={index}>
                        <Image src={image?.url} width={120} height={120}
                        className="rounded-lg object-cover" alt="images"/>
                    </div>
                ))}
            </div>
